...
public class SetOrderStatus implements ActionCommand {
    private Logger logger = LogManager.getLogger(SetOrderStatus.class);

    @Override
    public String execute(HttpServletRequest req) {
        String page;
        String orderStatusParam = req.getParameter("order_status");
        String orderIdToUpdate = req.getParameter("order_id_to_update");
        try {
            ServiceFactory.INSTANCE.getOrderService().updateOrderStatus(orderIdToUpdate, orderStatusParam);
