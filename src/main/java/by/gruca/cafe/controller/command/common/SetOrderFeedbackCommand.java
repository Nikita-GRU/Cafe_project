...
public class SetOrderFeedbackCommand implements ActionCommand {
    Logger logger = LogManager.getLogger(SetOrderFeedbackCommand.class);

    @Override
    public String execute(HttpServletRequest req) {
        String page;
        String orderIdParam = req.getParameter("order_id");
        String feedbackParam = req.getParameter("feedback");
        String scoreParam = req.getParameter("score");

        try {
            ServiceFactory.INSTANCE.getOrderService().updateOrderFeedback(orderIdParam, feedbackParam, scoreParam);
